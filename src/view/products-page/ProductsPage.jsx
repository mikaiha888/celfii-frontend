...
const ProductsPage = () => {
  ...
  const updateFilters = (newFilters) => {
    const { category, ...updatedFilters } = { ...filters, ...newFilters };
    setFilters({ category, ...updatedFilters });
    saveToLocalStorage("filters", updatedFilters);
    const urlFilters = (({ name, page }) => ({ name, page }))({ category, ...updatedFilters });
    ...
    navigate({
      pathname: category ? `/productos/${category}` : "/productos",
    ...
    <div className="min-h-screen mt-5">
      <div className="flex items-center justify-between gap-10 mb-10 font-medium">
        ...
        <div className="flex items-center gap-6">
          ...
          <SearchBar onSearch={(name) => updateFilters({ name })} variant="card" />
        </div>
      </div>
